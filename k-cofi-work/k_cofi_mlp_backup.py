...
class KCOFI_MLP(object):
	...
	def __init__(self, parameters, data_set, max_inner_length):
		...
		with tf.name_scope('embeddings_weights'):
			...
			self.user_embeddings = tf.Variable(tf.truncated_normal(shape = [parameters.n+1, 21], mean = 0.0, stddev = 0.01), name = 'user_embeddings', dtype = tf.float32)
			self.item_embeddings = tf.Variable(tf.truncated_normal(shape = [parameters.m+1, 21], mean = 0.0, stddev = 0.01), name = 'item_embeddings', dtype = tf.float32)
			self.gram_item_embeddings = tf.Variable(tf.truncated_normal(shape = [parameters.m+1, 22], mean = 0.0, stddev = 0.01), name = 'gram_item_embeddings', dtype = tf.float32)
			...
			self.h = tf.Variable(tf.truncated_normal(shape = [parameters.layers[-1], parameters.rating_types_num], mean = 0.0, stddev = 0.01), name = 'h_weights', dtype = tf.float32) # h:shape(8, 1)
			self.bias = tf.Variable(tf.zeros(parameters.rating_types_num), name = 'bias') # 0.0
	...
	def kcofi_mlp_model(self, parameters):
		with tf.name_scope('model'):
			...
			# self.output = tf.add(tf.to_float(tf.argmax(tf.sigmoid(tf.add(tf.matmul(self.a, self.h), self.bias)), axis = 1, output_type = tf.int32)), 1.0) # sigmoid(x) = 1 / (1 + e^(-x)), shape(?, rating_types_num)
		
		# with tf.name_scope('loss'):
		# 	self.loss = tf.reduce_mean(tf.square(tf.reduce_mean(self.rating, 1) - self.output))	

			self.output = tf.reduce_sum(tf.sigmoid(tf.add(tf.matmul(self.a, self.h), self.bias)), 1)

			self.output = tf.where(self.output>=1.0, self.output, tf.ones([self.train_num]))

			self.output = tf.where(self.output<=5.0, self.output, 5*tf.ones([self.train_num]))
...
def main(model, data_set, parameters, users_input, items_input, ratings_input, gram_items_input, factors_input, train_pairs_number):
	with tf.Session(config = tf_config) as sess:
		...
		sess.run(tf.assign(model.gram_item_embeddings[0], np.zeros([22])))
