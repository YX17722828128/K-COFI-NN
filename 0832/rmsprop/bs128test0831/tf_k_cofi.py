...
class parameters(object):
	...
	def __init__(self, learning_rate, alpha):
		...
		self.patience = 10 # assume that there are 100 iterations, if it can not achieve better performance for consecutive 50 times, iteration process will exit
...
class KCOFI_MLP(object):
	...
	def kcofi_mlp_model(self, parameters): # 根据论文prediction rule设置self.output，根据objective function设置self.loss
		...
		with tf.name_scope('optimizer'):
			self.optimizer = tf.train.RMSPropOptimizer(learning_rate = parameters.learning_rate).minimize(self.loss)
